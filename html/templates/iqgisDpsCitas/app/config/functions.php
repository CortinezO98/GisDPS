<?php
    ...
    function notificacion_familias($enlace_db, $asunto, $referencia, $contenido, $nc_address, $nc_id_modulo="1", $nc_cc="", $nc_id_set_from="2", $nc_prioridad="Alta") {
        // PROGRAMAR NOTIFICACIÓN CORREO
          ...
          $nc_subject=$asunto;
          $nc_body=str_replace("'", '"', $contenido_correo);
          $nc_embeddedimage_ruta="".IMAGES_ROOT."firma-dps.png";
    ...
    function registro_log($enlace_db, $log_modulo = NULL, $log_tipo = NULL, $log_detalle = NULL, $array_log = NULL, $clog_registro_usuario=NULL) {
        if ($clog_registro_usuario=='' OR $clog_registro_usuario==NULL) {
          $clog_registro_usuario=$_SESSION[APP_SESSION.'_session_usu_id'];
        }

        $consulta_string_log = "INSERT INTO `administrador_log`(`clog_log_modulo`, `clog_log_tipo`, `clog_log_accion`, `clog_log_detalle`, `clog_user_agent`, `clog_remote_addr`, `clog_remote_host`, `clog_script`, `clog_registro_usuario`) VALUES (?,?,?,?,?,?,?,?,?)";
        $consulta_registros_log = $enlace_db->prepare($consulta_string_log);
        $consulta_registros_log->bind_param("sssssssss", $log_modulo, $log_tipo, $log_accion, $log_detalle_insert, $_SERVER['HTTP_USER_AGENT'], $_SERVER['REMOTE_ADDR'], $_SERVER['REMOTE_HOST'], $_SERVER['PHP_SELF'], $clog_registro_usuario);

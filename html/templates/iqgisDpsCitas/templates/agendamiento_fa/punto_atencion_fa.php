<?php
  ...
  $consulta_contar_string="SELECT COUNT(`gcpa_id`) FROM `gestion_citasfa_punto_atencion` LEFT JOIN `administrador_ciudades` AS TC ON `gestion_citasfa_punto_atencion`.`gcpa_municipio`=TC.`ciu_codigo` LEFT JOIN `administrador_usuario` AS TU ON `gestion_citasfa_punto_atencion`.`gcpa_registro_usuario`=TU.`usu_id` WHERE 1=1 ".$filtro_buscar."";
        ...
        <div class="content-wrapper">
          <div class="row">
            ...
            <div class="col-md-9 mb-1 text-end">
              <a href="punto_atencion_fa_crear?pagina=1&id=<?php echo $filtro_permanente; ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Escalados">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td class="p-1 text-center">
                          <a href="punto_atencion_fa_editar?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&reg=<?php echo base64_encode($resultado_registros[$i][0]); ?>" class="btn btn-warning btn-icon px-1 py-1 mb-1" title="Editar"><i class="fas fa-pen font-size-11"></i></a>
                          <a href="punto_atencion_fa_agenda?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&reg=<?php echo base64_encode($resultado_registros[$i][0]); ?>" class="btn btn-dark btn-icon px-1 py-1 mb-1" title="Agenda"><i class="fas fa-calendar-alt font-size-11"></i></a>
                          <?php if($permisos_usuario=="Administrador"): ?>
                            <!-- <a href="punto_atencion_fa_eliminar?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&reg=<?php echo base64_encode($resultado_registros[$i][0]); ?>" class="btn btn-danger btn-icon px-1 py-1 mb-1" title="Eliminar"><i class="fas fa-trash-alt font-size-11"></i></a> -->
                          <?php endif; ?>
                      </td>

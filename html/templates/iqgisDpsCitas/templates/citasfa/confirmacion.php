...
// ini_set('display_errors', '1');
    ...
    $fecha_cita=validar_input(base64_decode($_GET['id']));
    $hora_cita=validar_input(base64_decode($_GET['id2']));
    $punto_cita=validar_input(base64_decode($_GET['id3']));
    ...
    if ($consecutivo=='' AND $doc_identidad=='') {
        if($confirma=='token' AND $tipo=='agendar' AND validar_input($datos['tipo_documento'])!='' AND validar_input($datos['numero_identificacion'])!='' AND validar_input($datos['nombres'])!='' AND validar_input($datos['correo'])!='' AND validar_input($datos['celular'])!=''  AND validar_input($datos['discapacidad'])!='' AND validar_input($datos['autoriza'])!=''){
            $consulta_string_validar="SELECT `gca_id`, `gca_punto`, `gca_usuario`, `gca_semana`, `gca_fecha`, `gca_hora`, `gca_estado`, `gca_estado_agenda`, TP.`gcpa_punto_atencion`, TP.`gcpa_direccion`, TU.`usu_nombres_apellidos` FROM `gestion_citasfa_agenda` LEFT JOIN `gestion_citasfa_punto_atencion` AS TP ON `gestion_citasfa_agenda`.`gca_punto`=TP.`gcpa_id` LEFT JOIN `administrador_usuario` AS TU ON `gestion_citasfa_agenda`.`gca_usuario`=TU.`usu_id` WHERE `gca_fecha`=? AND `gca_hora`=? AND `gca_punto`=? AND `gca_estado_agenda`='Disponible' LIMIT 1";

            $consulta_registros_validar = $enlace_db->prepare($consulta_string_validar);
            $consulta_registros_validar->bind_param("sss", $fecha_cita, $hora_cita, $punto_cita);
            $consulta_registros_validar->execute();
            $resultado_registros_validar = $consulta_registros_validar->get_result()->fetch_all(MYSQLI_NUM);

            if (count($resultado_registros_validar)>0) {
                $id_cita=$resultado_registros_validar[0][0];

                $consulta_string_agenda="SELECT `gca_id`, `gca_punto`, `gca_usuario`, `gca_semana`, `gca_fecha`, `gca_hora`, `gca_estado`, `gca_estado_agenda`, TP.`gcpa_punto_atencion`, TP.`gcpa_direccion`, TU.`usu_nombres_apellidos` FROM `gestion_citasfa_agenda` LEFT JOIN `gestion_citasfa_punto_atencion` AS TP ON `gestion_citasfa_agenda`.`gca_punto`=TP.`gcpa_id` LEFT JOIN `administrador_usuario` AS TU ON `gestion_citasfa_agenda`.`gca_usuario`=TU.`usu_id` WHERE `gca_id`=?";
                ...
                if ($resultado_registros_agenda[0][7]=='Disponible') {
                    $consulta_consecutivo = mysqli_query($enlace_db, "SELECT MAX(`gcar_consecutivo`) FROM `gestion_citasfa_agenda_reservas`");
                    ...
                    $gcar_datos_tipo_documento=validar_input($datos['tipo_documento']);
                    $gcar_datos_numero_identificacion=validar_input($datos['numero_identificacion']);
                    $gcar_datos_nombres=validar_input($datos['nombres']);
                    $gcar_datos_correo=validar_input($datos['correo']);
                    $gcar_datos_celular=validar_input($datos['celular']);
                    $gcar_datos_fijo=validar_input($datos['fijo']);
                    $gcar_datos_discapacidad=validar_input($datos['discapacidad']);
                    $gcar_datos_discapacidad_tipo=validar_input($datos['tipo_discapacidad']);
                    $gcar_datos_autoriza=validar_input($datos['autoriza']);
                    $gcar_observaciones='';
                    $gcar_atencion_usuario='';
                    $gcar_atencion_fecha='';
                    $gcar_radicado='';
                    $gcar_estado='Reservada';
                    $gcar_atencion_preferencial='';
                    $gcar_informacion_poblacional='';
                    $gcar_genero='';
                    ...
                    $sentencia_insert_cita = $enlace_db->prepare("INSERT INTO `gestion_citasfa_agenda_reservas`(`gcar_consecutivo`, `gcar_cita`, `gcar_punto`, `gcar_usuario`, `gcar_datos_tipo_documento`, `gcar_datos_numero_identificacion`, `gcar_datos_nombres`, `gcar_datos_correo`, `gcar_datos_celular`, `gcar_datos_fijo`, `gcar_datos_discapacidad`, `gcar_datos_discapacidad_tipo`, `gcar_datos_autoriza`, `gcar_observaciones`, `gcar_atencion_usuario`, `gcar_atencion_fecha`, `gcar_radicado`, `gcar_atencion_preferencial`, `gcar_informacion_poblacional`, `gcar_genero`, `gcar_nivel_escolaridad`, `gcar_envio_encuesta`, `gcar_celular`, `gcar_cancela_fecha`, `gcar_estado`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");

                    // Agrega variables a sentencia preparada
                    $sentencia_insert_cita->bind_param('sssssssssssssssssssssssss', $gcar_consecutivo, $gcar_cita, $gcar_punto, $gcar_usuario, $gcar_datos_tipo_documento, $gcar_datos_numero_identificacion, $gcar_datos_nombres, $gcar_datos_correo, $gcar_datos_celular, $gcar_datos_fijo, $gcar_datos_discapacidad, $gcar_datos_discapacidad_tipo, $gcar_datos_autoriza, $gcar_observaciones, $gcar_atencion_usuario, $gcar_atencion_fecha, $gcar_radicado, $gcar_atencion_preferencial, $gcar_informacion_poblacional, $gcar_genero, $gcar_nivel_escolaridad, $gcar_envio_encuesta, $gcar_celular, $gcar_cancela_fecha, $gcar_estado);
                    ...
                    if ($sentencia_insert_cita->execute()) {
                        ...
                        if (comprobarSentencia($enlace_db->info)) {
                            ...
                            $contenido="<p>Buen día, <b>".validar_input($datos['nombres'])."</b><br></p>
                                <p>De manera atenta, le confirmamos que su solicitud de <b>asignación</b> de cita para realizar inscripción al programa Familias en Acción de <b>Prosperidad Social</b> ha sido agendada, le recordamos presentarse en la siguiente fecha y lugar:</p>
                                <ul>
                              ...
                              <p>Recuerde:</p>
                                <ul>
                                    <li style='padding-left: 10px;'>Asistir con Original y fotocopia de la cédula de ciudadanía de quien se presenta como titular. En caso de que el titular sea menor de edad, debe presentar la tarjeta de identidad. En caso de que el titular no cuente con el original del documento de identidad, se acepta la contraseña. Para las personas con ciudadanía diferente a la colombiana, se acepta cédula de extranjería con fecha vigente.</li>
                                    <li style='padding-left: 10px;'>Original y fotocopia de los documentos de identidad para cada NNA menor de 18 años (registro civil y tarjeta de identidad para los NNA de 7 a 17 años).</li>
                                    <li style='padding-left: 10px;'>Contar con información de las entidades de educación donde se encuentran matriculados los niños, niñas y/o adolescentes, así como información de la Institución Prestadora del Servicio de salud - IPS que atiende tanto al titular como a todos los niños, niñas y/o adolescentes a inscribir. En ningún caso es necesario llevar soportes o certificados de esta información. Quien se presente como titular debe brindar esta información en el momento de la inscripción.</li>
                                    <li style='padding-left: 10px;'>Llegar al punto de inscripción con 10 minutos de anticipación.</li>
                              ...
                              <p>Le recordamos que usted puede consultar, reagendar o cancelar su cita, ingresando en la siguiente página web: <a href='https://agendamientodps.grupoasd.com.co/citasfa/inicio'>https://agendamientodps.grupoasd.com.co/citasfa/inicio</a></p>
                            ...
                            $nc_address=validar_input($datos['correo']).";";
                            $nc_cc="";
                            ...
                            $contenido_sms="Estimado (a) ".validar_input($datos['nombres']).", su cita para inscripción a Familias en Acción de radicado ".$gcar_consecutivo." en el punto ".$resultado_registros_agenda[0][8]." - ".$resultado_registros_agenda[0][9]." el ".$array_dias_nombre[date('N', strtotime($resultado_registros_agenda[0][4]))].' '.date('d', strtotime($resultado_registros_agenda[0][4])).' de '.$array_meses[intval(date('m', strtotime($resultado_registros_agenda[0][4])))].' de '.date('Y', strtotime($resultado_registros_agenda[0][4]))." a las ".date('h:i A', strtotime($resultado_registros_agenda[0][5])).", ha sido agendada con éxito. Revise su correo para más información.";
                            $nsms_url='';
                            $nsms_destino=validar_input($datos['celular']);
                            $estado_notificacion_sms=notificacion_agendamiento_sms($enlace_db, $nsms_identificador, $nsms_destino, $contenido_sms, $nsms_url, '19');
                            if ($estado_notificacion_sms) {
                                $estado_sms=1;
                            }

                            header("Location:confirmacion?cons=".base64_encode($gcar_consecutivo)."&doc=".base64_encode($gcar_datos_numero_identificacion));
                        } else {

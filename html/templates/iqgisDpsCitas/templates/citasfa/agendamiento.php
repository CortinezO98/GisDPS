...
// ini_set('display_errors', '1');
    ...
    if(isset($_POST["buscar_punto"])){
        ...
        $consulta_string_punto_atencion="SELECT `gcpa_id`, `gcpa_regional`, `gcpa_municipio`, `gcpa_punto_atencion`, `gcpa_direccion` FROM `gestion_citasfa_punto_atencion` WHERE `gcpa_municipio`=? AND `gcpa_estado`='Activo' ORDER BY `gcpa_punto_atencion`";
    ...
    $consulta_string_habilitado="SELECT `gch_id`, `gch_departamento`, `gch_municipio` FROM `gestion_citasfa_habilitados` WHERE `gch_documento`=? AND `gch_tipo_documento`=?";
    $consulta_registros_habilitado = $enlace_db->prepare($consulta_string_habilitado);
    $consulta_registros_habilitado->bind_param("ss", validar_input($datos_mostrar['numero_identificacion']), validar_input($datos_mostrar['tipo_documento']));
    $consulta_registros_habilitado->execute();
    $resultado_registros_habilitado = $consulta_registros_habilitado->get_result()->fetch_all(MYSQLI_NUM);

    if (count($resultado_registros_habilitado)>0) {
        $consulta_string_reserva="SELECT `gcar_consecutivo`, `gcar_cita`, `gcar_punto`, `gcar_usuario`, `gcar_datos_tipo_documento`, `gcar_datos_numero_identificacion`, `gcar_datos_nombres`, `gcar_datos_correo`, `gcar_datos_celular`, `gcar_datos_fijo`, `gcar_datos_autoriza`, `gcar_registro_fecha`, TP.`gcpa_punto_atencion`, TP.`gcpa_direccion`, TU.`usu_nombres_apellidos`, TC.`gca_fecha`, TC.`gca_hora` FROM `gestion_citasfa_agenda_reservas` LEFT JOIN `gestion_citasfa_punto_atencion` AS TP ON `gestion_citasfa_agenda_reservas`.`gcar_punto`=TP.`gcpa_id` LEFT JOIN `administrador_usuario` AS TU ON `gestion_citasfa_agenda_reservas`.`gcar_usuario`=TU.`usu_id` LEFT JOIN `gestion_citasfa_agenda` AS TC ON `gestion_citasfa_agenda_reservas`.`gcar_cita`=TC.`gca_id` WHERE `gcar_datos_numero_identificacion`=? AND TC.`gca_estado_agenda`='Reservada' AND TC.`gca_fecha`>=?";

        $consulta_registros_reserva = $enlace_db->prepare($consulta_string_reserva);
        $consulta_registros_reserva->bind_param("ss", validar_input($datos_mostrar['numero_identificacion']), date('Y-m-d'));
        ...
        if ($citas_agendadas==0) {
            $consulta_string_departamento="SELECT `ciu_codigo`, `ciu_departamento`, `ciu_municipio` FROM `administrador_ciudades` WHERE `ciu_codigo`=?";
            $consulta_registros_departamento = $enlace_db->prepare($consulta_string_departamento);
            $consulta_registros_departamento->bind_param("s", $resultado_registros_habilitado[0][2]);
                        ...
                        <div class="card-body">
                            ...
                            <div class="card">
                              ...
                              <div class="card-body py-2">
                                ...
                                <p class="card-text my-0"><b>¿Presenta discapacidad?:</b> <?php echo $datos_mostrar['discapacidad']; ?></p>
                                <?php if($datos_mostrar['discapacidad']=='Si'): ?>
                                    <p class="card-text my-0"><b>Discapacidad:</b> <?php echo $datos_mostrar['tipo_discapacidad']; ?></p>
                                <?php endif; ?>
                              </div>
                            </div>
                        </div>
                      ...
                      <div class="card card-rounded">
                        <div class="card-body">
                            ...
                            <form name="buscar_punto" action="" method="POST" enctype="multipart/form-data">
                                <div class="row">
                                    <?php if (count($resultado_registros_habilitado)>0): ?>
                                        <?php if ($citas_agendadas==0): ?>
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group my-1">
                                                    ...
                                                    <select class="form-control form-control-sm form-select" name="departamento_filtro" id="departamento_filtro" required>
                                                      <option value="">Seleccione</option>
                                                      <?php for ($i=0; $i < count($resultado_registros_departamento); $i++): ?>
                                                        <option value="<?php echo $resultado_registros_departamento[$i][0]; ?>" <?php if(isset($_POST["buscar_punto"]) AND $departamento_filtro==$resultado_registros_departamento[$i][0]){ echo "selected"; } ?>><?php echo $resultado_registros_departamento[$i][1]; ?></option>
                                                      <?php endfor; ?>
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="col-md-6">
                                                <div class="form-group my-1">
                                                    ...
                                                    <select class="form-control form-control-sm form-select" name="municipio_filtro" id="municipio_filtro" required>
                                                      <option value="">Seleccione</option>
                                                      <?php for ($i=0; $i < count($resultado_registros_departamento); $i++): ?>
                                                        <option value="<?php echo $resultado_registros_departamento[$i][0]; ?>" <?php if(isset($_POST["buscar_punto"]) AND $municipio_filtro==$resultado_registros_departamento[$i][0]){ echo "selected"; } ?>><?php echo $resultado_registros_departamento[$i][2]; ?></option>
                                                      <?php endfor; ?>
                                                    </select>
                                                </div>
                                            </div>
                                    ...
                                    <?php else: ?>
                                        <div class="col-md-12">
                                            <div class="card text-center">
                                              ...
                                              <div class="card-body">
                                                <p class="card-text">Estimado usuario, no hemos encontrado sus datos dentro de la base de familias habilitadas.</p>
                                                <div class="form-group">
                                                    <a href="inicio" class="btn btn-primary">Aceptar</a>
                                                </div>
                                              </div>

...
// ini_set('display_errors', '1');
    ...
    if ($consecutivo=='' AND $doc_identidad=='') {
        if($confirma=='token' AND $tipo=='reagendar-agendamiento' AND $id_consecutivo!='' AND $fecha_cita!='' AND $hora_cita!=''){
            ...
            if (count($resultado_registros_validar)>0) {
                ...
                if ($resultado_registros_agenda[0][7]=='Disponible') {
                    ...
                    $consulta_actualizar_agenda = $enlace_db->prepare("UPDATE `gestion_citasfa_agenda` SET `gca_estado_agenda`=? WHERE `gca_id`=? AND `gca_estado_agenda`='Disponible'");
                    ...
                    if (comprobarSentencia($enlace_db->info)) {
                        ...
                        // Prepara la sentencia
                        $sentencia_insert_cita = $enlace_db->prepare("INSERT INTO `gestion_citasfa_agenda_reservas`(`gcar_consecutivo`, `gcar_cita`, `gcar_punto`, `gcar_usuario`, `gcar_datos_tipo_documento`, `gcar_datos_numero_identificacion`, `gcar_datos_nombres`, `gcar_datos_correo`, `gcar_datos_celular`, `gcar_datos_fijo`, `gcar_datos_autoriza`, `gcar_observaciones`, `gcar_atencion_usuario`, `gcar_atencion_fecha`, `gcar_radicado`, `gcar_atencion_preferencial`, `gcar_informacion_poblacional`, `gcar_genero`, `gcar_nivel_escolaridad`, `gcar_envio_encuesta`, `gcar_celular`, `gcar_cancela_fecha`, `gcar_estado`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");

                        // Agrega variables a sentencia preparada
                        $sentencia_insert_cita->bind_param('sssssssssssssssssssssss', $gcar_consecutivo, $gcar_cita, $gcar_punto, $gcar_usuario, $gcar_datos_tipo_documento, $gcar_datos_numero_identificacion, $gcar_datos_nombres, $gcar_datos_correo, $gcar_datos_celular, $gcar_datos_fijo, $gcar_datos_autoriza, $gcar_observaciones, $gcar_atencion_usuario, $gcar_atencion_fecha, $gcar_radicado, $gcar_atencion_preferencial, $gcar_informacion_poblacional, $gcar_genero, $gcar_nivel_escolaridad, $gcar_envio_encuesta, $gcar_celular, $gcar_cancela_fecha, $gcar_estado);
                        ...
                        if ($sentencia_insert_cita->execute()) {
                            ...
                            $contenido="<p>Buen día, <b>".$gcar_datos_nombres."</b><br></p>
                                <p>De manera atenta, le confirmamos que su solicitud de <b>Reagendamiento</b> de cita para realizar inscripción al programa Familias en Acción de <b>Prosperidad Social</b> ha sido registrada, le recordamos presentarse en la siguiente fecha y lugar:</p>
                                <ul>
                            ...
                            // Prepara la sentencia
                            $consulta_actualizar_reserva = $enlace_db->prepare("UPDATE `gestion_citasfa_agenda_reservas` SET `gcar_estado`=?, `gcar_cancela_fecha`=? WHERE `gcar_consecutivo`=?");
                            ...
                            // Prepara la sentencia
                            $consulta_actualizar_agenda_anterior = $enlace_db->prepare("UPDATE `gestion_citasfa_agenda` SET `gca_estado_agenda`=? WHERE `gca_id`=? AND `gca_estado_agenda`='Reservada'");
                            ...
                            $gca_estado_agenda='Disponible';
                            // Prepara la sentencia
                            $consulta_actualizar_agenda = $enlace_db->prepare("UPDATE `gestion_citasfa_agenda` SET `gca_estado_agenda`=? WHERE `gca_id`=? AND `gca_estado_agenda`='Reservada'");

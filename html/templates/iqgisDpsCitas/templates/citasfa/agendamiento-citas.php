...
// ini_set('display_errors', '1');
    ...
    $consulta_string_habilitado="SELECT `gch_id`, `gch_departamento`, `gch_municipio` FROM `gestion_citasfa_habilitados` WHERE `gch_documento`=?";
    $consulta_registros_habilitado = $enlace_db->prepare($consulta_string_habilitado);
    $consulta_registros_habilitado->bind_param("s", validar_input($datos_mostrar['numero_identificacion']));
                                                          ...
                                                          <div class="accordion-body row">
                                                                <?php for ($j=0; $j < count($array_fechas_hora[$array_fechas[$i]]); $j++): ?>
                                                                    <div class="col-md-6 enlace-punto">
                                                                        <a href="confirmacion?t=<?php echo base64_encode($tipo); ?>&d=<?php echo $datos; ?>&id=<?php echo base64_encode($array_fechas[$i]); ?>&id2=<?php echo base64_encode($array_fechas_hora[$array_fechas[$i]][$j]); ?>&id3=<?php echo base64_encode($id_punto); ?>&confim=<?php echo base64_encode('token'); ?>" class="">

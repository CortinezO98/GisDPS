...
// ini_set('display_errors', '1');
    ...
    if ($consecutivo=='' AND $doc_identidad=='') {
        if($confirma=='token' AND $tipo=='agendar' AND validar_input($datos['tipo_documento'])!='' AND validar_input($datos['numero_identificacion'])!='' AND validar_input($datos['nombres'])!='' AND validar_input($datos['correo'])!='' AND validar_input($datos['celular'])!='' AND validar_input($datos['autoriza'])!=''){
            ...
            if ($resultado_registros_agenda[0][7]=='Disponible') {
                ...
                $gcar_atencion_preferencial=validar_input($datos['preferencial']);
                $gcar_informacion_poblacional=validar_input($datos['poblacional']);
                ...
                $gcar_genero=validar_input($datos['genero']);
                $gcar_nivel_escolaridad=validar_input($datos['escolaridad']);
                $gcar_envio_encuesta='';
                $gcar_celular='';
                $gcar_cancela_fecha='';
                $gcar_cancela_motivo='';
                ...
                if ($sentencia_insert_cita->execute()) {
                    ...
                    if (comprobarSentencia($enlace_db->info)) {
                        ...
                        $contenido="<p>Buen día, <b>".validar_input($datos['nombres'])."</b><br></p>
                            <p>De manera atenta, le confirmamos que su solicitud de <b>asignación</b> de cita para atención presencial en <b>Prosperidad Social</b> ha sido agendada, le recordamos presentarse en la siguiente fecha y lugar:</p>
                            <ul>

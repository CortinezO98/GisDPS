<?php
  ...
  $url_salir="punto_atencion?pagina=".$pagina."&id=".$filtro_permanente;
    ...
    unset($_SESSION[APP_SESSION.'_registro_eliminado_agendamiento_agenda']);

    if(isset($_POST["guardar_registro"])){
        $semana_nueva=validar_input($_POST['semana_nueva']);

        $consulta_string_agenda="SELECT DISTINCT `gca_semana` FROM `gestion_citas_agenda` WHERE `gca_punto`=? AND `gca_semana`=?";
        ...
        if (count($resultado_registros_agenda)==0) {
            header("Location:punto_atencion_agenda_configurar?pagina=".$pagina."&id=".$filtro_permanente."&reg=".base64_encode($id_registro)."&sem=".base64_encode($semana_nueva));
        } else {
                    ...
                    <div class="card-body">
                      <div class="row">
                        ...
                        <div class="col-md-12 mb-1">
                            <div class="form-group">
                                <button class="btn btn-success float-end ms-1" type="submit" name="guardar_registro">Crear agenda</button>
                        ...
                        <div class="col-md-12">
                            <?php if($permisos_usuario=="Administrador"): ?>
                              <a href="punto_atencion_agenda_eliminar?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&reg=<?php echo base64_encode($id_registro); ?>&sem=<?php echo base64_encode($resultado_registros_agenda[$i][0]); ?>" class="btn btn-danger btn-icon px-1 py-1 mb-1 float-start" title="Eliminar"><i class="fas fa-trash-alt font-size-11"></i></a>
                            <?php endif; ?>
                            <a href="punto_atencion_agenda_configurar?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&reg=<?php echo base64_encode($id_registro); ?>&sem=<?php echo base64_encode($resultado_registros_agenda[$i][0]); ?>" class="btn btn-dark d-block mb-1 py-1 text-start float-start"><?php echo date('Y', strtotime($resultado_registros_agenda[$i][0])).' - Semana '.date('W', strtotime($resultado_registros_agenda[$i][0])).' | '.date('d/m', strtotime("first day", strtotime($resultado_registros_agenda[$i][0].'0'))).' al '.date('d/m', strtotime("first day", strtotime($resultado_registros_agenda[$i][0].'6'))); ?></a>
                        </div>
                        <?php endfor; ?>
                      </div>
                    </div>

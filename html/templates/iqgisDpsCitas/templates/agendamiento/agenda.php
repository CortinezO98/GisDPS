<?php
  ...
  unset($_SESSION[APP_SESSION.'_registro_cancelado_agendamiento']);
  ...
  $consulta_contar_string="SELECT COUNT(`gca_id`) FROM `gestion_citas_agenda` LEFT JOIN `gestion_citas_punto_atencion` AS TP ON `gestion_citas_agenda`.`gca_punto`=TP.`gcpa_id` LEFT JOIN `administrador_usuario` AS TU ON `gestion_citas_agenda`.`gca_usuario`=TU.`usu_id` LEFT JOIN `administrador_ciudades` AS TC ON TP.`gcpa_municipio`=TC.`ciu_codigo` WHERE 1=1 ".$filtro_buscar."";
          ...
          <div class="row">
            ...
            <div class="col-md-9 mb-1 text-end">
              <?php if($permisos_usuario=="Administrador"): ?>
                <a href="agenda_cancelar?pagina=1&id=<?php echo $filtro_permanente; ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Cancelar Agenda">
                  <i class="fas fa-calendar-times btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Cancelar Agenda</span>
                </a>
              <?php endif; ?>

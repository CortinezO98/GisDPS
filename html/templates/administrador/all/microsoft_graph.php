<?php
  ...
  for ($i=0; $i < count($resultado_configuracion); $i++) { 
    ...
    $mail->tenant        = $ncr_tenant;        // Azure Active Directory Tenant ID
    ...
    if ($ncr_device_code=="") { // obtiene device_code
      ...
      // ✅ REMEDIACIÓN Stored XSS:
      // En lugar de hacer print_r directo (que puede incrustar HTML/JS),
      // se convierte a texto y se escapa antes de enviarlo al navegador.
      $jtoken_debug = print_r($jtoken, true);
      echo "<pre>";
      echo htmlspecialchars($jtoken_debug, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
      echo "</pre>";

      // Si más adelante quieres volver a activar la actualización de DB,
      // esta parte está lista para usarse de forma segura.
    ...
    if ($ncr_device_code!="" AND $ncr_token=="") { // obtiene token inicial con device_code
      ...
      // // Si necesitas depuración, hazlo escapando:
      // // $jtoken_debug = print_r($jtoken, true);
      // // echo "<pre>" . htmlspecialchars($jtoken_debug, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . "</pre>";
    ...
    if ($ncr_token!="" AND $ncr_token_refresh!="") { // refresca token con refresh_token

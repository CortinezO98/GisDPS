...
function actualizar_estado_sms(mysqli $db, string $estado, string $fecha, int $intentos, ?string $observaciones, int $id): void {
    ...
    if ($stmt = $db->prepare($sql)) {
        ...
        error_log('[notificaciones_citas_sms] Error preparando UPDATE: ' . $db->error);
    }
}
...
$query = "
 ...
 LEFT JOIN `administrador_buzones_sms` AS TR
     ...
     WHERE (`nsms_estado_envio`='Pendiente' OR `nsms_estado_envio`='Error de autenticación')
       AND (`nsms_id_modulo`='13' OR `nsms_id_modulo`='R13')
        ...
        if ($destino !== "" && $mensaje !== "" && $api_url !== "" && $api_user !== "" && $api_pass !== "") {
            ...
            try {
                ...
                if ($estado_error_final !== '') {
                    actualizar_estado_sms($enlace_db, $estado_error_final, $marca_temporal, $num_intentos, $mensaje_error, $id_notificacion);
                } else {
                    actualizar_estado_sms($enlace_db, $estado_error, $marca_temporal, $num_intentos, $mensaje_error, $id_notificacion);
                }

                error_log('[notificaciones_citas_sms] RequestException id=' . $id_notificacion . ' code=' . $code . ' msg=' . $mensaje_error);

            } catch (Exception $e) {
                $mensaje_error = $e->getMessage();
                actualizar_estado_sms($enlace_db, $estado_error, $marca_temporal, $num_intentos, $mensaje_error, $id_notificacion);
                error_log('[notificaciones_citas_sms] Exception id=' . $id_notificacion . ' msg=' . $mensaje_error);
            }

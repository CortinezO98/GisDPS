<?php
  ...
  $url_salir="citas_fa?pagina=".$pagina."&id=".$filtro_permanente."&bandeja=".base64_encode($bandeja)."&fecha=".base64_encode($fecha);
  ...
  if(isset($_POST["guardar_registro"])){
    ...
    if($_SESSION[APP_SESSION.'_registro_creado_atencion_cita']!=1){
      ...
      $consulta_actualizar_reserva = $enlace_db->prepare("UPDATE `gestion_citasfa_agenda_reservas` SET `gcar_observaciones`=?,`gcar_atencion_usuario`=?,`gcar_atencion_fecha`=?,`gcar_radicado`=?, `gcar_atencion_preferencial`=?, `gcar_informacion_poblacional`=?, `gcar_genero`=?, `gcar_nivel_escolaridad`=?, `gcar_envio_encuesta`=?, `gcar_celular`=?, `gcar_estado`=? WHERE `gcar_consecutivo`=?");

      // Agrega variables a sentencia preparada
      $consulta_actualizar_reserva->bind_param('ssssssssssss', $observaciones, $_SESSION[APP_SESSION.'_session_usu_id'], date('Y-m-d H:i:s'), $radicado, $atencion_preferencial_insert, $informacion_poblacional_insert, $gcar_genero, $gcar_nivel_escolaridad, $gcar_envio_encuesta, $gcar_celular, $estado, $id_registro);
  ...
  $array_informacion_poblacional[]='Gitano (Población Room)';
  $array_informacion_poblacional[]='Indígena';
  $array_informacion_poblacional[]='Mestizo';
  $array_informacion_poblacional[]='Afrocolombiano';
  $array_informacion_poblacional[]='Raizal';
  $array_informacion_poblacional[]='Palenquero';
  $array_informacion_poblacional[]='Comunidades negras';
  $array_informacion_poblacional[]='LGTBI';
  $array_informacion_poblacional[]='Otro Grupo';
  $array_informacion_poblacional[]='Ninguna de las anteriores';
  $array_informacion_poblacional[]='No aporta';

  $array_atencion_preferencial[]='Niñas/ Niños / Adolescentes';
  $array_atencion_preferencial[]='Adulto Mayor';
  $array_atencion_preferencial[]='Población Desplazada';
  $array_atencion_preferencial[]='Mujer Gestante';
  $array_atencion_preferencial[]='Persona con Discapacidad';
  $array_atencion_preferencial[]='Víctima del Conflicto Armado';
  $array_atencion_preferencial[]='Periodista';
  $array_atencion_preferencial[]='Ninguna de las anteriores';
  $array_atencion_preferencial[]='No aporta';

  if (!isset($informacion_poblacional)) {
    $informacion_poblacional=array();
  }
  if (!isset($atencion_preferencial)) {
    $atencion_preferencial=array();
  }
?>
<!DOCTYPE html>
...
<body class="sidebar-dark sidebar-icon-only">
                  ...
                  <div class="card card-rounded">
                    <div class="card-body">
                      <div class="row">
                        ...
                        <div class="col-md-12 d-none" id="atencion_preferencial_div">
                            <div class="form-group my-1">
                                <label for="atencion_preferencial" class="my-0">Atención preferencial</label>
                                <select class="selectpicker form-control form-control-sm form-select" data-live-search="false" data-container="body" name="atencion_preferencial[]" id="atencion_preferencial" multiple <?php if($_SESSION[APP_SESSION.'_registro_creado_atencion_cita']==1) { echo 'disabled'; } ?> required disabled>
                                    
                                </select>
                            </div>
                        </div>
                        <div class="col-md-12 d-none" id="informacion_poblacional_div">
                            <div class="form-group my-1">
                                <label for="informacion_poblacional" class="my-0">Información poblacional</label>
                                <select class="selectpicker form-control form-control-sm form-select" data-live-search="false" data-container="body" name="informacion_poblacional[]" id="informacion_poblacional" multiple <?php if($_SESSION[APP_SESSION.'_registro_creado_atencion_cita']==1) { echo 'disabled'; } ?> required disabled>
  ...
  <script type="text/javascript">
    function validar_estado(){
        ...
        var informacion_poblacional_opciones ='<?php for ($i=0; $i < count($array_informacion_poblacional); $i++): ?>
                                      <option class="font-size-11" value="<?php echo $array_informacion_poblacional[$i]; ?>"><?php echo $array_informacion_poblacional[$i]; ?></option>\
                                    <?php endfor; ?>';

        var atencion_preferencial_opciones ='<?php for ($i=0; $i < count($array_atencion_preferencial); $i++): ?>
                                      <option value="<?php echo $array_atencion_preferencial[$i]; ?>"><?php echo $array_atencion_preferencial[$i]; ?></option>\
                                    <?php endfor; ?>';
        ...
        if(estado=="Asiste") {
            ...
            $("#informacion_poblacional_div").removeClass('d-none').addClass('d-block');
            $("#atencion_preferencial_div").removeClass('d-none').addClass('d-block');

            $('#informacion_poblacional').html("");
            $('#informacion_poblacional').selectpicker('destroy');
            $('#informacion_poblacional').selectpicker('refresh');
            $('#informacion_poblacional').html(informacion_poblacional_opciones);
            $('#informacion_poblacional').selectpicker('refresh');

            $('#atencion_preferencial').html("");
            $('#atencion_preferencial').selectpicker('destroy');
            $('#atencion_preferencial').selectpicker('refresh');
            $('#atencion_preferencial').html(atencion_preferencial_opciones);
            $('#atencion_preferencial').selectpicker('refresh');
        } else {
            ...
            $("#informacion_poblacional_div").removeClass('d-block').addClass('d-none');
            $("#atencion_preferencial_div").removeClass('d-block').addClass('d-none');
            $('#informacion_poblacional').selectpicker('refresh');
            $('#atencion_preferencial').selectpicker('refresh');
        }
    }

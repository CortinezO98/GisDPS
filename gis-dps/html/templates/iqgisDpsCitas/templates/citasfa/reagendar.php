    ...
    if ($tipo=='reagendar') {
        if(isset($_POST["buscar_cita"])){
            ...
            if ($tipo_documento!="" AND $numero_identificacion!="" AND $_POST['g-recaptcha-response']!="") {
                ...
                if ($captcha_response) {
                    $consulta_string_reserva="SELECT `gcar_consecutivo`, `gcar_cita`, `gcar_punto`, `gcar_usuario`, `gcar_datos_tipo_documento`, `gcar_datos_numero_identificacion`, `gcar_datos_nombres`, `gcar_datos_correo`, `gcar_datos_celular`, `gcar_datos_fijo`, `gcar_datos_autoriza`, `gcar_registro_fecha`, TP.`gcpa_punto_atencion`, TP.`gcpa_direccion`, TU.`usu_nombres_apellidos`, TC.`gca_fecha`, TC.`gca_hora`, TC.`gca_estado_agenda` FROM `gestion_citasfa_agenda_reservas` LEFT JOIN `gestion_citasfa_punto_atencion` AS TP ON `gestion_citasfa_agenda_reservas`.`gcar_punto`=TP.`gcpa_id` LEFT JOIN `administrador_usuario` AS TU ON `gestion_citasfa_agenda_reservas`.`gcar_usuario`=TU.`usu_id` LEFT JOIN `gestion_citasfa_agenda` AS TC ON `gestion_citasfa_agenda_reservas`.`gcar_cita`=TC.`gca_id` WHERE `gcar_datos_tipo_documento`=? AND `gcar_datos_numero_identificacion`=? AND `gcar_estado`='Reservada' AND TC.`gca_fecha`>=? ORDER BY `gcar_consecutivo` DESC LIMIT 1";
                                        ...
                                        <?php if (isset($_POST["buscar_cita"]) AND count($resultado_registros_reserva)>0 AND $citas_disponibles>0 AND $captcha_response): ?>
                                            <div class="card text-center">
                                              <div class="card-body">
                                                ...
                                                <div class="form-group">
                                                    <a href="reagendar-agendamiento?t=<?php echo base64_encode('reagendar-agendamiento'); ?>&con=<?php echo base64_encode($resultado_registros_reserva[0][0]); ?>" class="btn btn-primary">Reagendar Cita</a>
                                                    <a href="inicio" class="btn btn-primary">Salir</a>
                                                </div>
                                              </div>
                                            </div>
                                        <?php elseif(isset($_POST["buscar_cita"]) AND $citas_disponibles==0 AND $captcha_response): ?>
                                            <div class="card text-center">
                                              <div class="card-body">

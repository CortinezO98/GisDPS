<?php
    ...
    if(isset($_POST["reporte"])){
        ...
        $consulta_string="SELECT `gkc_id`, `gkc_kiosco`, `gkc_boton`, `gkc_datos_tipo_documento`, `gkc_datos_numero_identificacion`, `gkc_datos_nombres`, `gkc_datos_correo`, `gkc_datos_celular`, `gkc_datos_fijo`, `gkc_datos_autoriza`, `gkc_atencion_preferencial`, `gkc_informacion_poblacional`, `gkc_registro_fecha`, TB.`gkpb_nombre`, TP.`gkp_titulo` FROM `gestion_kioscos_consultas` LEFT JOIN `gestion_kioscos_programas_boton` AS TB ON `gestion_kioscos_consultas`.`gkc_boton`=TB.`gkpb_id` LEFT JOIN `gestion_kioscos_programas` AS TP ON TB.`gkpb_programa`=TP.`gkp_id` WHERE 1=1 AND `gkc_boton`=? AND `gkc_registro_fecha`>=? AND `gkc_registro_fecha`<=? ORDER BY `gkc_id` ASC";

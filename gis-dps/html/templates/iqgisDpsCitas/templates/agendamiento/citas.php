<?php
  ...
  if($bandeja=="Pendientes"){
      ...
      if ($permisos_usuario=="Administrador") {
          ...
          $consulta_string_punto_atencion="SELECT `gcpau_punto_atencion`, `gcpau_usuario` FROM `gestion_citas_punto_atencion_usuario` WHERE `gcpau_usuario`=?";
  ...
  $consulta_contar_string="SELECT COUNT(`gcar_consecutivo`) FROM `gestion_citas_agenda_reservas` LEFT JOIN `gestion_citas_punto_atencion` AS TP ON `gestion_citas_agenda_reservas`.`gcar_punto`=TP.`gcpa_id` LEFT JOIN `administrador_usuario` AS TU ON `gestion_citas_agenda_reservas`.`gcar_usuario`=TU.`usu_id` LEFT JOIN `gestion_citas_agenda` AS TC ON `gestion_citas_agenda_reservas`.`gcar_cita`=TC.`gca_id` WHERE 1=1 AND TC.`gca_fecha`>=? AND TC.`gca_fecha`<=? ".$filtro_buscar." ".$filtro_perfil." ".$filtro_bandeja."";
  ...
  <div class="container-scroller">
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
        <div class="content-wrapper">
          <div class="row">
            ...
            <div class="col-md-7 mb-1 text-end">
              <a href="citas?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Pendientes'); ?>&fecha=<?php echo base64_encode($filtro_fecha_permanente); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Pendientes">
                <i class="fas fa-user-clock btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Pendientes</span>
              </a>
              <a href="citas?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Cerrados'); ?>&fecha=<?php echo base64_encode($filtro_fecha_permanente); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Cerrados">
                <i class="fas fa-user-lock btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Cerrados</span>
              </a>
              <a href="citas?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Mi grupo'); ?>&fecha=<?php echo base64_encode($filtro_fecha_permanente); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Mi grupo">
                <i class="fas fa-users btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Mi grupo</span>
              </a>
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td class="p-1 text-center">
                        <?php if(($resultado_registros[$i][22]!="Cancelada" AND ($permisos_usuario=="Administrador" OR $permisos_usuario=="Gestor")) OR ($resultado_registros[$i][22]=="Reservada" AND $permisos_usuario=="Usuario")): ?>
                          <a href="citas_editar?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?><?php echo $parametros_add; ?>&reg=<?php echo base64_encode($resultado_registros[$i][0]); ?>" class="btn btn-warning btn-icon px-1 py-1 mb-1" title="Editar"><i class="fas fa-pen font-size-11"></i></a>
                        <?php endif; ?>
                      </td>
        ...
        <!-- modal reportes -->
        <?php require_once('citas_reporte.php'); ?>
        <!-- modal -->

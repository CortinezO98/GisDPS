<?php
  ...
  if(isset($_POST["guardar_registro"])){
    ...
    $consulta_actualizar = $enlace_db->prepare("UPDATE `gestion_citas_punto_atencion` SET `gcpa_regional`=?,`gcpa_municipio`=?,`gcpa_punto_atencion`=?,`gcpa_direccion`=?,`gcpa_estado`=? WHERE `gcpa_id`=?");

    // Agrega variables a sentencia preparada
    $consulta_actualizar->bind_param('ssssss', $regional, $ciudad, $punto_atencion, $direccion, $estado, $id_registro);
    ...
    if (comprobarSentencia($enlace_db->info)) {
        ...
        // Prepara la sentencia
        $sentencia_delete = $enlace_db->prepare("DELETE FROM `gestion_citas_punto_atencion_usuario` WHERE `gcpau_punto_atencion`=? AND `gcpau_usuario`=?");

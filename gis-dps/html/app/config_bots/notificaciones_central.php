...
function actualizar_estado_envio(mysqli $db, string $estado, string $fecha, int $intentos, int $idCorreo): void {
    ...
    if ($stmt) {
        ...
        error_log('[notificaciones_central] Error preparando UPDATE: ' . $db->error);
    }
}
    ...
    foreach ($resultado_notificaciones as $row) {
        ...
        if ($host !== "" && $port !== "" && $secure !== "" && $smtpAuth !== "" &&
            ...
            try {
                ...
                // TO
                foreach (array_filter(explode(";", $to_raw)) as $addr) {
                    $addr = trim($addr);
                    if ($addr !== "") {
                ...
                error_log('[notificaciones_central] Excepción id=' . $id_correo . ' -> ' . $reporte_error);
            }
        } else {

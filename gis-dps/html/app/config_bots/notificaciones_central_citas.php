<?php
// notificaciones_central_citas.php (versión remediada 2nd-order SQLi, sin cambiar la lógica de negocio)
...
function actualizar_estado_envio(mysqli $db, string $estado, string $fecha, int $intentos, int $idCorreo): void {
    ...
    if ($stmt) {
        ...
        error_log('[noti_citas] Error preparando UPDATE: ' . $db->error);
    }
}
    ...
    foreach ($resultado_notificaciones as $row) {
        ...
        if ($host !== "" && $port !== "" && $secure !== "" && $smtpAuth !== "" &&
            ...
            try {
                ...
                error_log('[noti_citas] Excepción id='.$id_correo.' -> '.$reporte_error);
            }
